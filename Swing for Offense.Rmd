...
```{r}
baseModel <- randomForest(BinaryOffense ~ ., data = trainSet, ntree = 50)
```
...
# The plot below shows the error message that the model was producing us. The errors seem a but high therefore in the tuned model the number of trees will have to be increased.
...
```{r}
newModel <- randomForest(BinaryOffense ~ Pull. + Oppo. + Cent. + GB. + FB. + LD. + Hard. + Soft. + Med. + Swing. + K., data = trainSet, ntree = 100, maxnodes = 30)
```

```{r}
OffenseTrain <- predict(newModel, trainSet, type = "response")
OffenseTest <- predict(newModel, testSet, type = "response")
```
...
# This error plot is much better than the one above. Error distribution flatlines around 80 trees so 100 seemed to be a good number here.
...
roc_train <- roc(ifelse(trainSet$OffensePred == "1", "1", "0"), as.numeric(trainSet$OffensePred))
